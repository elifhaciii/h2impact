#!/bin/bash
# Bash script to run PyPSA-Eur with an H2-enabled configuration and copy the result .nc file

set -e

if [ "$#" -ne 1 ]; then
    echo "Usage: bash $0 <path-to-config-H2-yaml>"
    exit 1
fi
...
# Result file pattern (H2 enabled)
RESULT_SRC="results/${run_name}/postnetworks/base_s_${clusters}___${planning_horizon}.nc"
RESULT_DST="../../results/${country}-${cutout}-hydrogen-result.nc"
